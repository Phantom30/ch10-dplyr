...
nba_turnover <- mutate(nba, steals_ratio = TOV / STL)

# Sort the teams from lowest turnover/steal ratio to highest
# Which team has the lowest turnover/steal ratio?
sorted <- arrange(nba_turnover,steals_ratio)
nba_turnover %>%
...
offense_stats <- select(assists_per_game,Team , ORB, FG., AST)
...
defense_stats <- select(assists_per_game,Team, DRB, STL, BLK)
...
better_shooters <- function ( team_one, team_two){
  better_team <- filter(nba, Team == c(team_one, team_two))%>%
    filter(FG. == max(FG.))%>%
    select(Team, FG., PTS)
...
# Call the function on two teams to compare them (remember the `*` if needed)
  better.shooters <- better_shooters("Golden State Warriors*", "Utah Jazz")
